...
> [!WARNING]
> **외부망 웹 UI 접속 시 토큰 필수 (보안 강화):**
> 로컬 홈 네트워크(사설 IP 대역) 외부의 외부망(LTE/외부 와이파이)에서 웹 플레이어 화면(`3005` 포트)에 접속할 때는, URL 뒤에 반드시 구성 탭에서 지정한 보안 토큰 파라미터를 추가해야 합니다.
> - **접속 주소 예시**: `http://<HA_외부_IP_또는_도메인>:3005/?token=<MY_TOKEN>`
> - 토큰 없이 외부망에서 접속하면 보안 수칙에 의해 **`403 Forbidden`** 오류와 함께 접근이 차단되며, 5회 이상 오인증 실패 누적 시 5분 동안 접속 IP가 강제로 임시 밴(Ban) 처리됩니다.
